...
| `restore` | Moves every externally-stored app — and its `~/Library` entries — back to its original location on the internal disk. Entries where real data reappeared at home resolve the same two-copy conflict instead of silently keeping local: identical or clearly staler copies resolve automatically (the loser goes to the Trash, recoverably), divergent copies ask which to keep. |
